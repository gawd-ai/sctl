...
use super::history::RelayConnectionHistory;
...
pub fn relay_router(relay_state: RelayState) -> Router {
    ...
    //
    // Only transports that cannot ride a JSON request/response frame keep
    // named routes: /ws upgrades to a streaming WebSocket, and the STP chunk
    // route carries binary tunnel frames. Every other device endpoint rides
